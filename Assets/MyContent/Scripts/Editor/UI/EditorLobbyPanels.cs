...
public class EditorLobbyPanels : InspectorEditor
{
    ...
    private const string TITLE = "PANEL MANAGER";
    private const string SUBTITLE_PANELS = "PANELS";
    private const string SUBTITLE_ELEMENTS = "ELEMENTS";

    private const string ERROR_ENABLE_IN_RUN_MODE = "this can't edited in run-mode.";
    private const string ERROR_OBJECT_NULL = "Set the field to avoid errors.";
    private const string WARNING_OBJECT_TO_DISABLE = "Set disable this GameObject.";
    private const string WARNING_OBJECT_TO_ENABLE = "Set active this GameObject.";
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox(ERROR_ENABLE_IN_RUN_MODE, MessageType.Error);
            return;
        }
        ...
        /** PANELS **/
        Subtitle(SUBTITLE_PANELS);
        FieldPanels();

        /** ELEMENTS **/
        Rect(1, Color.gray);
        Subtitle(SUBTITLE_ELEMENTS, H7);
        FieldElements();
    }
    ...
    private void FieldPanels()
    {
        ...
        if (!loginPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (!loginPanel.activeSelf && loginPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_ENABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!selectionPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (selectionPanel.activeSelf && selectionPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!createRoomPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (createRoomPanel.activeSelf && createRoomPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!randomRoomPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (randomRoomPanel.activeSelf && randomRoomPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!listRoomPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (listRoomPanel.activeSelf && listRoomPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!howToPlayPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (howToPlayPanel.activeSelf && howToPlayPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!creditsPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (creditsPanel.activeSelf && creditsPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!insideRoomPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (insideRoomPanel.activeSelf && insideRoomPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
        ...
        if (!loadingPanel) EditorGUILayout.HelpBox(ERROR_OBJECT_NULL, MessageType.Error);
        if (loadingPanel.activeSelf && loadingPanel) EditorGUILayout.HelpBox(WARNING_OBJECT_TO_DISABLE, MessageType.Warning);
        EditorGUILayout.Space();
    }
